...
/**
  ...
  */
object WeightedQuickUnionWithPathCompression extends App {
  ...
  val expectedNodeArray = List(6, 6, 6, 6, 6, 6, 6, 6, 4, 4)
  val endTime = System.currentTimeMillis

  def root(childNode: Int): Int = if (childNode != nodeArray(childNode)) {
    val parent = root(nodeArray(childNode));
    println(s"parent of $childNode is $parent");
    nodeArray(childNode) = parent;
    parent
  } else childNode
  ...
  union(6, 5);
  union(5, 0)
  assert(find(0, 5)) //true
  assert(!find(4, 6)) //false


  union(2, 1);
  union(7, 2)
  assert(find(1, 7)) //true
  assert(!find(4, 6)) //false
  assert(!find(2, 6)) //false
  assert(!find(2, 4)) //false

  union(6, 1);
  union(7, 3)
  assert(find(1, 7)) //true
  assert(find(0, 3)) //true
  assert(find(1, 6)) //true
  assert(find(4, 6)) //true
  assert(find(1, 4)) //true
  assert(find(7, 3), "nodes 7 and 3 are connected and must return true")
  assert(find(7, 3), "nodes 7 and 3 are connected and must return true")
  assert(find(7, 3), "nodes 7 and 3 are connected and must return true")
  assert(find(7, 3), "nodes 7 and 3 are connected and must return true")
